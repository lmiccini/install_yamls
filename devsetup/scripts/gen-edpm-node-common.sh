...
# Add region suffix to node names (e.g., edpm-compute-0-region2)
...
EDPM_COMPUTE_NAME=${EDPM_COMPUTE_NAME:-"edpm-${EDPM_SERVER_ROLE}-${EDPM_COMPUTE_SUFFIX}${REGION_SUFFIX}"}
...
DISK_FILENAME=${DISK_FILENAME:-"edpm-${EDPM_SERVER_ROLE}-${EDPM_COMPUTE_SUFFIX}${REGION_SUFFIX}.qcow2"}
...
if [ $EDPM_EMULATED_NVME_ENABLED == "true" ] ; then
    NVME1_FILENAME=${NVME1_FILENAME:-"edpm-${EDPM_SERVER_ROLE}-${EDPM_COMPUTE_SUFFIX}${REGION_SUFFIX}-nvme1.qcow2"}
    ...
    NVME2_FILENAME=${NVME2_FILENAME:-"edpm-${EDPM_SERVER_ROLE}-${EDPM_COMPUTE_SUFFIX}${REGION_SUFFIX}-nvme2.qcow2"}
